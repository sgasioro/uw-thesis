...
Preliminary results are presented here for the gluon-gluon fusion non-resonant search. These results will be combined with a VBF channel as discussed, but this is left for future work. Limits are set for $\kappa_{\lambda}$ values 
from $-20$ to $20$.   The cross section limit for $HH$ production is set at \SI{180}{\fb} (expected), corresponding to 
$5.9$ (expected) times the Standard Model prediction. $\kappa_{\lambda}$ is constrained to be within the range $-3.9 \leq \kappa_{\lambda} \leq 10.9$ (expected) at 95~\% CL. These results are shown in Figure \ref{fig:nonres-limits}.

\begin{figure}[ht]
  \centering
  \subfloat{
    \label{fig:limits-kl}
    \includegraphics[width=0.8\textwidth]{figures/exp-limit-2x2.pdf}
  }
  \caption{\label{fig:nonres-limits}
    Expected (dashed black)  95\% CL upper limits on the cross-section times branching ratio of non-resonant production 
    ...
    The cross section limit for $HH$ production is set at \SI{180}{\fb} (expected), corresponding to $5.9$ (expected) times the Standard Model prediction. $\kappa_{\lambda}$ is constrained to be within the range $-3.9 \leq \kappa_{\lambda} \leq 10.9$ (expected). The nominal \(H \rightarrow b\bar{b}\) branching ratio is taken as 0.582.
  }
\end{figure}
